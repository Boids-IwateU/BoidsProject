...
public class RadarControl : MonoBehaviour
{
  ...
  List<GameObject> iconMaker(List<Vector3> list, GameObject icon)
  {
    ...
    foreach(var member in list)
    {
      ...
      newicon.transform.localPosition = member;
  ...
  void Start()
  {
    ...
    enemyIcons = iconMaker(posTransfer(neighborsCheck("boids")), enemyIcon);
    baitIcons = iconMaker(posTransfer(neighborsCheck("bait")), enemyIcon);
  ...
  void Update()
  {
    iconRemainer(posTransfer(neighborsCheck("boids")), enemyIcon, enemyIcons);
    ...
    playerIcons.transform.localPosition = new Vector3(0, 0, 1);
    playerIcons.transform.localScale = new Vector3(1, 1, 1);
